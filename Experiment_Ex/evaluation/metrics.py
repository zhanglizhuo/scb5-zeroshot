...
def compute_hit_at_k(
    logits: np.ndarray,       # (N, C) float — 原始相似度分数
    labels_multilabel: np.ndarray,  # (N, C) binary
    k: int = 1,
) -> float:
    """
    Hit@k：top-k 预测中至少有一个命中任意真实标签
    """
    N = logits.shape[0]
    topk_indices = np.argsort(logits, axis=1)[:, -k:]  # (N, k)
    hits = 0
    for i in range(N):
...
def compute_multilabel_metrics(
    ...
    labels: np.ndarray,         # (N, C) binary
    threshold: float = 0.20,    # 相似度阈值（归一化后）
) -> Dict:
    """计算多标签 Sample-F1, Macro-F1, Micro-F1"""
    # 归一化 logits 到 [0, 1]
...
def bootstrap_ci(
    ...
    *args,
    ...
    """
    对任意指标函数计算 Bootstrap 95% 置信区间
    metric_fn(*args) -> float
    """
...
def multilabel_to_primary(labels_multilabel: np.ndarray) -> np.ndarray:
    """
    多标签 → 主标签（取最小 class index）
    对于每个样本，主标签 = min(正标签 index)
    """
...
def full_evaluation(
    logits: np.ndarray,           # (N, C) cosine similarity scores
    labels_multilabel: np.ndarray,# (N, C) binary or (N,) int
    num_classes: int,
    multilabel: bool,
    class_names: Optional[List[str]] = None,
