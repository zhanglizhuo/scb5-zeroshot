...
# Build pseudo-logits from predicted_labels, then compute hit@k.
...
"$PY" scb5_zeroshot/Experiment_Ex/main_mllm.py \
  ...
  --results_dir "$HR_DIR" > /tmp/qwen35_27b_hr.log 2>&1 &
PID_HR=$!
...
"$PY" scb5_zeroshot/Experiment_Ex/main_mllm.py \
  ...
  --results_dir "$BT_DIR" > /tmp/qwen35_27b_bt.log 2>&1 &
PID_BT=$!
...
root = Path('/home/broadsense/works/lizhuo/AutoResearchClaw')
summary_tb = root / 'scb5_zeroshot/Experiment_Ex/results/mllm_qwen35_27b_tb/mllm_summary.json'
summary_hr = root / 'scb5_zeroshot/Experiment_Ex/results/mllm_qwen35_27b_hr/mllm_summary.json'
summary_bt = root / 'scb5_zeroshot/Experiment_Ex/results/mllm_qwen35_27b_bt/mllm_summary.json'
out_dir = root / 'scb5_zeroshot/Experiment_Ex/results/mllm_qwen35_27b_full'
out_dir.mkdir(parents=True, exist_ok=True)

with open(summary_tb) as f:
    tb = json.load(f)['qwen35_ollama']
with open(summary_hr) as f:
    hr = json.load(f)['qwen35_ollama']
with open(summary_bt) as f:
    bt = json.load(f)['qwen35_ollama']
...
echo "Teacher logs: /tmp/qwen35_27b_tb_s0.log /tmp/qwen35_27b_tb_s1.log /tmp/qwen35_27b_tb_s2.log"
echo "Other logs: /tmp/qwen35_27b_hr.log /tmp/qwen35_27b_bt.log"
