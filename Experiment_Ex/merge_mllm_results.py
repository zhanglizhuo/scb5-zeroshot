...
Usage:
  python3 scb5_zeroshot/Experiment_Ex/merge_mllm_results.py \
    --input_glob 'scb5_zeroshot/Experiment_Ex/results/mllm*' \
    --out scb5_zeroshot/Experiment_Ex/results/mllm_merged_summary.json
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--input_glob",
        type=str,
        default="scb5_zeroshot/Experiment_Ex/results/mllm*",
        help="Glob pattern for result directories that contain *_preds.json",
    )
    p.add_argument(
        "--out",
        type=str,
        default="scb5_zeroshot/Experiment_Ex/results/mllm_merged_summary.json",
...
def main() -> None:
    args = parse_args()
    matched = sorted(Path(p) for p in glob.glob(args.input_glob))
