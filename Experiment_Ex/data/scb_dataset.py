...
DATASETS_ROOT = REPO_ROOT / "datasets_scb"
SUBSET_LOCAL_DIRS = {
...
def _auto_find_datasets_scb() -> Optional[str]:
    """尝试自动向上查找名为 `datasets_scb` 的目录或使用环境变量 `SCB_DATA_DIR`。"""
    env = os.environ.get("SCB_DATA_DIR")
    if env and os.path.exists(env):
        return env

    # 从当前文件位置向上查找几层，寻找 datasets_scb
    p = Path(__file__).resolve()
    for i in range(1, 6):
        if i - 1 >= len(p.parents):
            break
        candidate = p.parents[i - 1] / "datasets_scb"
        if candidate.exists():
...
class SCBDataset(Dataset):
    ...
    def _load_data(self, data_dir, hf_repo, cache_dir) -> List[Dict]:
        ...
        # 若没有传入，尝试使用环境变量或自动查找工作区下的 datasets_scb
        auto_dir = _auto_find_datasets_scb()
